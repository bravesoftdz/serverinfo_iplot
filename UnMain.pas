...
const
 cr=8; //максимальное количество €дер процессора которое будет писать программа
...
(*  TNewSQLThread = class(TThread)
  ...
  TfileStruct=record
   time:Double;    //врем€ в миллисекундах
   data:Double;   //данные
   core:byte;       //номер €дра
  end; //type
...
BEGIN
 ...
 //if MySQLClient.connect('tppasu04', 'pma', 'logotip','monitoring', 3306, '', false, 0) then begin
  ...
  //MessageBox(fmMain.Handle,'ѕодключились','',MB_OK); //if
 // MySQLClient.close;
 //end; //else MessageBox(fmMain.Handle,'Ќеудалось подключитьс€ к базе','',MB_OK); //if
END;*)
//конец потока =================================================================
...
// InitDriver;
 ...
 day:=IntToStr(yy)+'-'+AddZero(mm)+'-'+AddZero(dd); //дл€ имени файла
...
begin
 ...
 DecodeDateTime(Nw,yy,mm,dd,hh,mi,ss,aa);
// sec:=ss+(mi*60)+(hh*60*60); //будет как контрольна€ сумма
 day:=IntToStr(yy)+'-'+AddZero(mm)+'-'+AddZero(dd); //дл€ имени файла
 CurrentDay:=ExeP+'data\'+day+'.cpu';
...
begin
 ...
 Application.ProcessMessages;
 //если данные выбираютс€ за 1 дату
 ...
 end else begin //если данные выбираютс€ за разные даты
  DecodeDateTime(startdate.DateTime,yy,mm,dd,hh,mi,ss,aa);
  day:=ExeP+'data\'+IntToStr(yy)+'-'+AddZero(mm)+'-'+AddZero(dd)+'.cpu';
  sec1:=ss+(mi*60)+(hh*60*60);
